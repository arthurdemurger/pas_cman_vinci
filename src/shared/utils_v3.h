...
#define colorOff(void) printf("\033[0m");  // text color reset
...
// idem sem_down(void) with sem_num = 0
...
// idem sem_up(void) with sem_num = 0
 ...
 *      timeout : timeout in specifies the number of milliseconds that poll(void) should  block  
 ...
 *       then poll(void) blocks until one of the events occurs.
 *       The call will block until either:
 *       *  a file descriptor becomes ready;
 *       *  the call is interrupted by a signal handler; or
 *       *  the timeout expires.
 *       Specifying  a  negative value in timeout means an infinite timeout.  
 *       Specifying a timeout of zero   causes poll(void) to return immediately, 
